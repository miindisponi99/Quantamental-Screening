...
keywords_method1 = pd.read_excel('Duplicate_keywords.xlsx', header=0, sheet_name='Method1')
keywords_method4 = pd.read_excel('Duplicate_keywords.xlsx', header=0, sheet_name='Method4')
...
def fetch_form_accno():
    ...
    with open('accno_data.pkl', 'wb') as f:
...
def fetch_facts(cik_data, url_facts):
    ...
    with open('facts_data.pkl', 'wb') as f:
        pickle.dump((cik_data), f)

    return 'facts_data.pkl'
...
def fetch_concept_url(cik_data):
    ...
    with open('concept_urls.pkl', 'wb') as f:
        pickle.dump(positive_responses, f)

    return 'concept_urls.pkl'
...
def fetch_concept(cik_data, concept_urls):
    ...
    with open('concept_data.pkl', 'wb') as f:
        pickle.dump(cik_data, f)

    return 'concept_data.pkl'
...
def dfs_facts_tickers(cik_data, bond_yield, filename_prefix):
    ...
    filename = f"{filename_prefix}_df_file.pkl"
...
def process_cik_data(cik_database, bond_yield, start_date, end_date):
    ...
    with open('frames_df_file.pkl', 'wb') as f:
        pickle.dump(cik_dfs, f)

    return 'frames_df_file.pkl'
...
def classify_multiple_ciks(cik_list, api_df=None, selenium_df=None):
    ...
    defensive_df_total = pd.concat(defensive_results, ignore_index=True) if defensive_results else pd.DataFrame(columns=['Asset'])
    enterprising_df_total = pd.concat(enterprising_results, ignore_index=True) if enterprising_results else pd.DataFrame(columns=['Asset'])
    ncav_df_total = pd.concat(ncav_results, ignore_index=True) if ncav_results else pd.DataFrame(columns=['Asset'])
    if errors:
        print("Errors:")
        for error in errors:
            print(error)
    print("Defensive Assets:")
    display(defensive_df_total) #type:ignore
    print("Enterprising Assets:")
    display(enterprising_df_total) #type:ignore
    print("NCAV Assets:")
    display(ncav_df_total) #type:ignore
...
def classify_stock_relaxed(df, company_price, ticker=None, multiple=False):
    ...
    if multiple:
        ...
        defensive_df = pd.DataFrame(defensive_assets, columns=['Asset'])
        enterprising_df = pd.DataFrame(enterprising_assets, columns=['Asset'])
        ncav_df = pd.DataFrame(ncav_assets, columns=['Asset'])
